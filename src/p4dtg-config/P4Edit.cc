...
P4Edit::P4Edit( DataSource *in_obj, QWidget *parent )
    : QDialog( parent, Qt::WindowTitleHint | Qt::WindowSystemMenuHint )
{
	obj = in_obj;
	setWindowModality( Qt::WindowModal );
	int new_p4 = !obj->nickname;
	if( new_p4 )
	    setWindowTitle( QString( tr( "Add Perforce Server Source" ) ) );
	else
	    setWindowTitle( QString( tr( "Edit Perforce Server Source" ) ) );
	...
	nickname_edit = new QLineEdit( QString( QUTF8( obj->nickname ) ) );
	if( !new_p4 )
	...
	if( !obj->get_attributes() )
	    attributes_btn->setEnabled( false );
	...
	if( obj->error )
	    status_edit->append( QString( QUTF8( " " ) ) +
				QString( QUTF8( obj->error ) ) );
	if( obj->warnings )
	    status_edit->append( QString( QUTF8( " " ) ) +
				QString( QUTF8( obj->warnings ) ) );
	ok_btn->setEnabled( false );
...
void P4Edit::setup_combos()
{
	...
	int which = 
	    moddate_combo->findText( QString( QUTF8( obj->moddate_field ) ) );
	...
	which = moduser_combo->findText( QString( QUTF8( obj->moduser_field)));
	moduser_combo->setCurrentIndex( which > -1 ? which : 0 );
}

void P4Edit::okPushed()
{
	if( !obj->nickname && 
...
void P4Edit::cancelPushed()
{
	done( 0 );
}

void P4Edit::helpPushed()
{
	global->help->showSource( P4Edit_help, this );
}

void P4Edit::attributesPushed()
{
...
void P4Edit::segmentPushed()
{
	    ...
	    obj->status != DataSource::READY )
	{
	    QMessageBox::information( this,
		QUTF8( "Perforce Defect Tracking Gateway" ), 
		QUTF8( "A valid connection to the Perforce server is required "
		"before editing segments.\n" ) );
	    return;
	}
	if( !obj->seg_ok )
	{
	    QMessageBox::information( this, 
		QUTF8( "Perforce Defect Tracking Gateway" ), 
		QUTF8( "The Perforce Server needs to have a DTG_MAPID field "
		"in the jobspec in order to support segmentation. See "
		"the User Guide or online help for details.\n" ) );
	    return;
	}
...
void P4Edit::moddateSwitched( const QString & /* text */ )
{
	enable_ok();
}

void P4Edit::moduserSwitched( const QString & /* text */ )
{
	enable_ok();
}

void P4Edit::nickChanged( const QString &text )
{
...
void P4Edit::serverChanged( const QString &text )
{
...
void P4Edit::userChanged( const QString &text )
{
...
void P4Edit::passChanged( const QString &text )
{
	if( text != pass )
