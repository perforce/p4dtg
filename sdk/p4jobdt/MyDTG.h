...
class MyDTGProj {
	...
	/* P4 Specific */
	struct DTGStrList *list_fixes( const char *defect, 
					struct DTGError *error );
	struct DTGFixDesc *describe_fix( const char *fixid, 
					struct DTGError *error );
	struct DTGStrList *find_defects( int limit, 
					const char *qual, 
					struct DTGError *error );
	int is_fix_pending( const char *fixid, struct DTGError *error );
	char *translate( const char *str, int sm, char *&err, 
			int rev = 0, int cpp = 1 );
	struct DTGFixDesc *translate( struct DTGFixDesc *fix, 
					struct DTGError *error );
	StrDict *translate( StrDict *fields, struct DTGError *error, 
			int rev = 0 );
	struct DTGStrList *translate( struct DTGStrList *list, 
					struct DTGError *error, 
					int rev = 0 );
	struct DTGFieldDesc *translate( struct DTGFieldDesc *list, 
					struct DTGError *error, 
					int rev = 0 );
