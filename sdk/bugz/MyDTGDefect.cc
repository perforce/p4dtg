...
struct DTGField *MyDTGDefect::get_fields( struct DTGError *error )
{
	...
	for( f = fields; f; f = f->next )
	    if( !strcmp( "Status", f->name ) )
	        s = f;
	    else if( !strcmp( "Resolution", f->name ) )
	        r = f;
	char *tmp;
	...
	list = append_DTGField( list, new_DTGField( "Status/Resolution", tmp ));
	delete[] tmp;

	// Create the "Product/Component" field
	for( f = fields; f; f = f->next )
	    if( !strcmp( "Product", f->name ) )
	        s = f;
	    else if( !strcmp( "Component", f->name ) )
	        r = f;
	if( r && r->value && *r->value )
	    tmp = mk_string( s->value, "/", r->value );
	else
	    tmp = mk_string( s->value );
	list = append_DTGField( list, new_DTGField( "Product/Component", tmp ));
	delete[] tmp;
...
char *MyDTGDefect::get_field( const char *name, struct DTGError *error )
{
	...
	if( !strcmp( "Status/Resolution", name ) )
	{
	    struct DTGField *s = NULL, *r = NULL;
	    for( struct DTGField *f = fields; f; f = f->next )
	        if( !strcmp( "Status", f->name ) )
	...
	if( !strcmp( "Product/Component", name ) )
	{
	    struct DTGField *s = NULL, *r = NULL;
	    for( struct DTGField *f = fields; f; f = f->next )
	        if( !strcmp( "Product", f->name ) )
	                    ...
	                    struct DTGError *error )
{
	...
	{
	    set_DTGError( error, 
		"MyDTGDefect::set_field: Product/Component is read-only");
	    return;
	}
